...
/// Converts a `Words64Sequence` representing an RLP (Recursive Length Prefix) encoded data into an
/// array of `RLPItem` structs.
///
/// # Arguments
/// * `rlp` - The `Words64Sequence` representing the RLP-encoded data.
///
/// # Returns
/// * `Array<RLPItem>` - The `RLPItem` structs representing the individual RLP elements within the
/// input sequence.
///
/// This function takes a `Words64Sequence` as input, which is expected to be an RLP-encoded data
/// structure.
/// It iterates over the RLP elements within the input sequence and constructs an array of `RLPItem`
/// structs, each representing a single RLP element.
...
/// # Arguments
/// * `rlp` - The `Words64Sequence` representing the RLP-encoded data.
/// * `position` - The position (index) within the `Words64Sequence` where the desired RLP element
/// is located.
///
/// # Returns
/// * `Words64Sequence` - The extracted RLP element data.
///
/// This function takes a `Words64Sequence` representing an RLP-encoded data structure and a
/// position within that sequence.
/// It extracts the RLP element located at the specified position and returns it as a new
/// `Words64Sequence`.
...
/// Helper function to parse the RLP element header and determine the position and length of an RLP
/// element.
///
/// # Arguments
/// * `rlp` - The `Words64Sequence` representing the RLP-encoded data.
...
/// This function takes a `Words64Sequence` representing an RLP-encoded data structure and a
/// position within that sequence.
/// It parses the RLP element header at the specified position and returns an `RLPItem` struct
/// containing the position, length, and first byte of the element.
...
/// Extracts a sequence of bytes from a `Words64Sequence` representing an RLP-encoded data
/// structure.
///
/// # Arguments
/// * `rlp` - The `Words64Sequence` containing the RLP-encoded data.
/// * `start` - The starting position (index) within the `Words64Sequence` from which to extract the
/// bytes.
...
/// the type and length of the item. If the first byte is greater than or equal to 192 (0xC0 in
/// hexadecimal), the item is considered an RLP list. Otherwise, it is a single value.
